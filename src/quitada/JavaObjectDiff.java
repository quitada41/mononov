...
public class JavaObjectDiff {
	...
	private final String CNT_SUBOPS = "instance";
	private final String CNT_SU_SUBOPS = "i";
	private final String BYT_SUBOPS = "byte";
	private final String BYT_SU_SUBOPS = "b";
	private final String CLS_SUBOPS = "class";
	private final String CLS_SU_SUBOPS = "c";
	private final String ORD_OPS = OPS_HEADER + "order";
	private final String ORD_SU_OPS = SU_OPS_HEADER + "o";
	private final String DES_SUBOPS = "des";
	private final String DES_SU_SUBOPS = "d";
	private final String ASC_SUBOPS = "asc";
	private final String ASC_SU_SUBOPS = "a";
	...
	public void runMerge(String[] argz) {
		...
		for (argi = 0 ; argi < argz.length ; argi++) {
			...
			try {
				...
				subops = argz[argi].substring(dindex + 1);
		...
		case 0:
			if (isDesOrd) {
				...
				so = ASCENDING;
				Arrays.sort(oa, new InstanceAscComparator());
			}
			break;
		case 1:
			st = "#bytes";
			if (isDesOrd) {
				...
				so = ASCENDING;
				Arrays.sort(oa, new ByteAscComparator());
			}
			break;
		case 2:
			st = "class name";
			if (isDesOrd) {
				Arrays.sort(oa, new ClassDesComparator());				
			} else {
				so = ASCENDING;
				Arrays.sort(oa, new ClassAscComparator());
			}
			break;
		}
		System.out.println("Sorted by " + st + " in " + so + " order.");
		/*if (isCount) {
			Arrays.sort(oa, new InstanceDesComparator());
	...
	private void showOptions() {
		System.out.println("Options:\n"
				...
				+ SPACES + "\tSub options:\n"
				+ SPACES + "\t" + SPACES + CNT_SUBOPS +" | " + CNT_SU_SUBOPS + "\t:Sort by #instances(DEFAULT behavior).\n"
				+ SPACES + "\t" + SPACES + BYT_SUBOPS +" | " + BYT_SU_SUBOPS + "\t:Sort by #bytes.\n"
				+ SPACES + "\t" + SPACES + CLS_SUBOPS +" | " + CLS_SU_SUBOPS + "\t:Sort by class name.\n"
				+ SPACES + ORD_OPS + SUBOPS_DELI + "<sub ops> | " + ORD_SU_OPS + SUBOPS_DELI + "<sub ops>\t:Specify sort order.\n"
				+ SPACES + "\tSub options:\n"
				+ SPACES + "\t" + SPACES + DES_SUBOPS +" | " + DES_SU_SUBOPS + "\t:Sort in descending order(DEFAULT behavior).\n"
				+ SPACES + "\t" + SPACES + ASC_SUBOPS +" | " + ASC_SU_SUBOPS + "\t:Sort in " + ASCENDING +" order.\n"
				);
	}
}
